table 130 "Incoming Document"
{
    ...
    local procedure GetPostedNAVRecord(var RelatedRecord: Variant): Boolean
    var
        RelatedRecordRef: RecordRef;
    begin
    ...
    local procedure FindPostedNAVRecord(var RelatedRecord: Variant): Boolean
    var
        SalesInvoiceHeader: Record "Sales Invoice Header";
        SalesCrMemoHeader: Record "Sales Cr.Memo Header";
        PurchInvHeader: Record "Purch. Inv. Header";
    ...
    local procedure GetUnpostedNAVRecord(var RelatedRecord: Variant): Boolean
    var
        RelatedRecordRef: RecordRef;
    begin
        if GetRelatedRecord(RelatedRecordRef) then begin
    ...
    local procedure FindUnpostedNAVRecord(var RelatedRecord: Variant): Boolean
    var
        SalesHeader: Record "Sales Header";
        PurchaseHeader: Record "Purchase Header";
        GenJournalLine: Record "Gen. Journal Line";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterUpdateDocumentFields(IncomingDocument: Record "Incoming Document";DocExists: Boolean)
    begin
    end;
