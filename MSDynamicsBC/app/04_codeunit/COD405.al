codeunit 405 "Graph Mail"
{
    ...
    begin
        ...
        MailSent := TrySendMail(TempEmailItem,RefreshToken);
        ...
        if GraphMailSetup.Get then begin
          GraphMailSetup.SetRefreshToken(RefreshToken);
          if GraphMailSetup.Modify then;
        end;
    end;

    [TryFunction]
    procedure TrySendMail(TempEmailItem: Record "Email Item" temporary;var RefreshToken: Text)
    var
        GraphMailSetup: Record "Graph Mail Setup";
    begin
        GraphMailSetup.Get;
        GraphMailSetup.SendMail(TempEmailItem,RefreshToken);
    end;
